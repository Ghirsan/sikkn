<div class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <flux:card class="!p-0">
        ...
        @else
            <flux:table>
                <flux:table.columns>
                    <flux:table.column>{{ __('Topik / Tanggal') }}</flux:table.column>
                    <flux:table.column>{{ __('Rangkuman & Feedback') }}</flux:table.column>
                    ...
                    @foreach($logs as $log)
                        <flux:table.row :key="$log->id">
                            <flux:table.cell>
                                ...
                                <div class="text-xs text-neutral-500">{{ $log->date->translatedFormat('d F Y') }}</div>
                            </flux:table.cell>
                            <flux:table.cell>
                                <div class="text-sm text-neutral-600">{{ $log->discussion_summary }}</div>
                                @if($log->dpl_feedback)
                                    <div class="mt-2 rounded-lg border border-green-200 bg-green-50 p-2 text-xs text-green-700 dark:border-green-800 dark:bg-green-900/20 dark:text-green-400">
                                        <strong>{{ __('Feedback DPL:') }}</strong> {{ $log->dpl_feedback }}
                                    </div>
                                @endif
                            </flux:table.cell>
                            <flux:table.cell>

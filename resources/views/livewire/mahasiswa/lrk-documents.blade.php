    ...
    @if($allApproved)
        <flux:card class="border-2 border-green-300 bg-green-50/50 dark:border-green-700 dark:bg-green-900/10">
            <div class="flex items-start gap-3">
                ...
                <div class="w-full">
                    <flux:heading size="lg">{{ __('PDF LRK Siap Dicetak!') }}</flux:heading>
                    <flux:text class="mt-1 text-sm text-neutral-600">{{ __('Seluruh rencana program kerja tim telah disetujui DPL.') }}</flux:text>
                    <div class="mt-3">
                        <flux:button href="{{ route('lrk.pdf', $group) }}" target="_blank" variant="filled" icon="document-arrow-down" size="sm">{{ __('Cetak PDF LRK') }}</flux:button>
                    </div>
                </div>
            </div>
        </flux:card>
    @else
        <flux:card class="border-2 border-dashed border-blue-300 bg-blue-50/50 dark:border-blue-700 dark:bg-blue-900/10">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <flux:heading size="lg">{{ __('PDF LRK Belum Siap') }}</flux:heading>
                    <flux:text class="mt-1 text-sm text-neutral-600">{{ __('Tombol cetak akan aktif setelah seluruh rencana program kerja disetujui DPL.') }}</flux:text>
                </div>
            </div>
        </flux:card>
    @endif

<x-layouts::app :title="__('Rancangan Dokumen')">
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
        ...
        <div class="grid auto-rows-min gap-4 md:grid-cols-4">
            ...
            <div class="rounded-xl border border-neutral-200 p-4 dark:border-neutral-700">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <flux:text class="text-sm text-neutral-500 dark:text-neutral-400">{{ __('Review DPL') }}</flux:text>
                        <flux:text class="text-2xl font-bold">0</flux:text>
                    </div>
                </div>
            </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <flux:input icon="magnifying-glass" placeholder="{{ __('Cari kelompok atau mahasiswa...') }}" size="sm" class="w-72" />
            <flux:select size="sm" placeholder="{{ __('Semua Status') }}" class="w-48">
                ...
                <flux:select.option>{{ __('Review DPL') }}</flux:select.option>
                <flux:select.option>{{ __('Disetujui DPL') }}</flux:select.option>
                <flux:select.option>{{ __('PDF Final') }}</flux:select.option>
            </flux:select>
        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700">
            <div class="border-b border-neutral-200 px-6 py-4 dark:border-neutral-700">
                <flux:heading size="lg">{{ __('Program Kerja Per Tim') }}</flux:heading>
            </div>

            {{-- Empty State --}}
            <div class="px-6 py-12 text-center">
                <flux:icon name="document-text" class="mx-auto size-12 text-neutral-300 dark:text-neutral-600" />
                <flux:heading size="lg" class="mt-4">{{ __('Belum Ada Rancangan Dokumen') }}</flux:heading>
                <flux:text class="mt-2 text-sm text-neutral-500 dark:text-neutral-400">
                    {{ __('Rancangan dokumen program kerja dari seluruh tim KKN akan tampil di sini setelah mahasiswa mulai mengusulkan program.') }}
                </flux:text>
            </div>
        </div>
    </div>
